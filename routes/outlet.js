...
//Directly Pushing the bill to the HQ
router.post('/DirectBill', function (req, res, next) {
    ...
    console.log("Got order details from outlet:" + queueItem.bill_no + "Outlet Id : " + queueItem.outlet_id);
...
//Pushing the Bills to redis queue to Check the missing bills
...
router.get('/get_outstanding_po/:id', function (req, res, next) {
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        // get all POs in the current day which are not received
        //the query shall group by po-id, batch-id, rest-id and sum the items and qty
        // left join purchase_order_batch pb on pb.purchase_order_id=po.id  and pb.Received_time is null \
        // current_date
...
as start_of_day \
from outlet where id=$1) od on od.id=outlet_id',
            [outlet_id],
            function (query_err, result) {
                if (query_err) {
                    handleError(client, done, res, 'error running query' + query_err);
